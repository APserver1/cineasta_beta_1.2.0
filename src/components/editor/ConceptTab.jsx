...
const ConceptTab = ({ project, onUpdateProject, readOnly = false, registerFlush }) => {
  ...
  useEffect(() => {
    if (!project?.concepto_data) return;
    const incoming = project.concepto_data;
  ...
  }, [project?.id, project?.concepto_data]);
  ...
  const handleFormChange = (newDataOrUpdater, immediate = false) => {
    if (readOnly) return;
    let nextData = null;
    setFormData(prev => {
      nextData = typeof newDataOrUpdater === 'function' ? newDataOrUpdater(prev) : newDataOrUpdater;
      latestFormDataRef.current = nextData;
      return nextData;
    });
    if (immediate) {
        saveData(nextData ?? latestFormDataRef.current);
    }
  };
  ...
  const closeEditor = () => {
      ...
      if (!readOnly) saveData(latestFormDataRef.current);
  };
      ...
      <div className="flex flex-1 min-h-0">
        <div className="w-1/2 min-h-0 border-r border-gray-200 bg-gray-50 relative" data-pins-surface="concept-map" data-pins-type="reactflow">
          <ConceptMap 
            ...
            onChange={(updates, immediate) => handleFormChange(prev => ({ ...prev, ...updates }), immediate)}
